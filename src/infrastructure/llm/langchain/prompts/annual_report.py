...
class AnnualReportPromptV1(BasePrompt):
    ...
    def _create_template(self) -> str:
        ...
        return """你是一位严谨、细致的数据提取专家，
你的任务是全面解析一份上市公司年度报告摘要，
并严格按照两步指令，提取公司元数据和业务概念信息，
最终输出一个完整的、结构化的JSON对象。
...
- `top_shareholders`: 从"股东情况"部分的"前10名股东持股情况表"中提取。
  对于每一行，提取"股东名称(全称)"和"持股比例(%)"，并形成一个包含`name`和`holding_percentage`的对象。
  最多提取10个。

## 第二步：提取业务概念信息

然后，通读全文，提取所有独立的业务概念，并填充到`business_concepts`数组中：
1.  **全面识别并拆分业务概念**: 如果一个业务板块包含多个差异巨大、面向不同市场的子业务
    （例如，助听器芯片和国产服务器CPU），请将它们拆分为独立的业务概念进行提取。
...
3.  **填充业务概念字段**:
    ...
    * `importance_score`: 基于该业务在报告中的描述，
      给出一个0到1.0的综合重要性评分（必须是0到1之间的小数，如0.85）。
    * `development_stage`: 必须从以下四个选项中选择一个：
      ["成熟期", "成长期", "探索期", "并购整合期"]。不要使用其他描述。
    * `timeline`: 仅当原文明确提及业务开始时间或近期关键事件时才填充对应字段，
      否则返回null。
    * `metrics`: 在此字段中，你只能填充在原文中找到的【具体的数值或百分比】。
      重要说明：
      - 所有数值必须是纯数字，不要包含单位或文字描述
      - 每个子字段（revenue, capacity, sales_volume等）必须是单个数字，不能是字典或对象
      - 如果一个业务有多种产品的产能，请选择总产能或主要产品的产能作为单一数值
      - 如"150%"应该填写150.0
      - 如"超过60%"应该填写60.0
    ...
    * `source_sentences`: 必须从原文中摘录2-3句最能支撑你本次提取结论的
      核心句子。
