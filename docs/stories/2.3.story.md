...
- Health check: See logger output in QwenServiceAdapter._check_service_health()
- Reranking performance metrics: Tracked via track_rerank_performance context manager
- HTTP retry logic: Logged in QwenServiceAdapter.rerank_documents()
...
19. Fixed line length issue in qwen_service_adapter.py
...
- src/infrastructure/llm/qwen/qwen_service_adapter.py (fixed bare except, logging)
...
- ✅ Resource cleanup implemented - `_reranker` properly closed in shutdown_dependencies()
...
- ✅ Adapter references corrected - Tests use QwenServiceAdapter
