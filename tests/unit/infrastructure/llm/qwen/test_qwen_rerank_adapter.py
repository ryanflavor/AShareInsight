"""Unit tests for QwenRerankAdapter."""
...
class TestQwenRerankAdapter:
    """Unit tests for Qwen Service HTTP adapter."""

    @pytest.fixture
    def config(self):
        """Create test configuration."""
        return QwenRerankConfig(
    ...
    @pytest.mark.asyncio
    @patch("src.infrastructure.llm.qwen.qwen_rerank_adapter.track_rerank_performance")
    async def test_rerank_documents_success(
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        # Mock json() as a regular method that returns a value directly
